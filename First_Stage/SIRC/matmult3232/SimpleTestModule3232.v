...
//%% File Name		: SimpleTestModule3232.v																		%%
//%% Title 		: SIRC FSM for Matrix Multiplication															%%
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
   ...
   // reg xyz;
	...
	///////////////////////////FSM Begins Here/////////////////////////////////////
			...
			case(currState)
				...
				RUN: 
				begin
					LED[2] <= 1;					countNumberOfCycles <= countNumberOfCycles + 1;
					//Logic to feed FIFO
					//If there is enough space in the fifo and there are more values to read, try to request a read the
					// next clock cycle
					if((currPendingReads + inputFifoCount < 1056) && inputDone == 0) 
					begin //1056 = 1024+32
						inputMemoryReadReq <= 1;
					end
					else begin
				...
				begin
					...
					// datatranspose <= inputFifoDataOut; // The FIFO data is fed to the input of the transpose block for the first 32 clock cycle
				end
				...
				begin
					...
					oa2_en_data	= 1;		 
							...
							begin
								...
								collect_resultOA2	<= 0;  
								if(blocks_computed == 31) // Since we are focussed on 1024 matrix calculation no of blocks = 1024
								begin
				...
				FILLOUTPUTFIFO : // This module is to fill the remaining last 32 words of data from OA block into the FIFO
					begin
						...
						begin
							counter3	<= counter3 + 1;
							// if(collect_resultOA1 == 1 && collect_resultOA2 == 0)  // Do Exactly the opposite of original
							// begin
									// outputfifo 	<= oa1_out32 ;
							// end else if(collect_resultOA1 == 0 && collect_resultOA2 == 1)
							// begin
									// outputfifo 	<= oa2_out32 ;
							// end 			
   ...
   //assign inFifoReadDataValid = (currState == FILLOUTPUTFIFO);
	...
	assign dataoutFIFO = (collect_resultOA1 == 1 && collect_resultOA2 == 0 )? oa1_out32 : ((collect_resultOA1 == 0 && collect_resultOA2 == 1)? oa2_out32 : 0);
